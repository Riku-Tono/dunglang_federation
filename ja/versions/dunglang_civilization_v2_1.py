...
===========================
発展版: Temporal / Archaeology / Civilization Federation
...
def main() -> None:
    ...
    print("  発展軸: Temporal Banana Rewrite / ArchaeologyIR / Civilization Federation")
    ...
    theorem_results = [
        GorillaTheoremProver.prove("おなら = 風"),
        GorillaTheoremProver.prove("おなら ≠ 風"),
        GorillaTheoremProver.prove("この定理は証明可能"),
    ]
    for r in theorem_results:
        print(f"  証明: {r['statement']} -> {r['result']} / {r['reason']}")
    ...
    for ev in store.archaeology:
        if ev.source_event not in official_details:
            leak = CausalityLeakEvent(
                step=ev.step,
                kind="causality_leak",
                detail=f"公式歴史にない痕跡を検出: {ev.source_event}",
                frontend="ArchaeologyIR",
                era="archaeological",
